...
class FeiFei:
    def __init__(self):
        ...
        self.explain_keyword = [
            [['是什么'], 'intro'],
            [['怎么用', '使用场景', '有什么作用', '有什么用'], 'usage'],
            [['怎么卖', '多少钱', '多少块', '售价', '价格', '价钱'], 'price'],
    ...
    def __auto_speak(self):
        ...
        while self.__running:
            ...
            try:
                ...
                # 简化逻辑：默认执行带货脚本，带货脚本执行其间有人互动，则执行完当前脚本就回应最后一条互动，回应完继续执行带货脚本
                if i <= 3 and len(self.interactive) > i:
                    ...
                    if index == 1:
                        ...
                        if flag == 1:
                            flag = 0
                            self.a_msg = ['欢迎{}'.format(user_name), '感谢{}的到来'.format(user_name),
                                          '欢迎{}来到我们的直播间'.format(user_name)][
                                random.randint(0, 2)]
                        else :
                            self.a_msg = ''
    ...
    def __update_mood(self, typeIndex):
        ...
        if typeIndex == 1:
            try:
                ...
                if result == 2:
                    self.mood = self.mood + (chat_perception / 400.0)
                elif result == 0:
            ...
            self.mood = self.mood + (perception["join"] / 500.0)
